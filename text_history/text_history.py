...
class TextHistory:
    ...
    def get_actions(self, from_version=None, to_version=None):
        ...
        return [act for act in self._data if from_version <= act.from_version and act.to_version <= to_version]   

        
    
class Action(ABC): # ABC - Abstract Base Class
...
class ReplaceAction(Action):
    ...
    def apply(self, string):
        return string[:self.pos] + self.text + string[self.pos + len(self.text):]
